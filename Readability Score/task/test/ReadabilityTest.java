    ...
    public CheckResult check(String reply, TestClue clue) {
        ...
        for (Object lineObj : reply.lines().toArray()) {
            ...
            if (line.contains("polysyllables:")) {
                String strPolysyllables = line.split(":")[1].strip();

                CheckResult wrongAnswer = new CheckResult(false,
                    "Wrong number of polysyllables. " +
                        "Should be " + clue.polysyllables + ", but found " + strPolysyllables);

                int polysyllables;

                try {
                    polysyllables = (int) Double.parseDouble(strPolysyllables);
